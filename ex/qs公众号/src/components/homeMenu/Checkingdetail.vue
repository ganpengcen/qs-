        ...
        <van-tab title="设备设施">
          <div v-for="item in dataarrs" :key="item.KeyID">
            <van-swipe-cell v-if="item.SubTypeName == '设备设施'"  :on-close="onClose" >
              <div  class="station" @click="taskResult(item.SubID,item.BillID,item.SubType,item.SubID,item.DangerID)">
                <div class="station1">
...
<script>
  ...
  export default {
    ...
    methods: {
      taskResult(e, b, SubType, SubID, DangerID) {
      ...
      //拿数据方法
      getSubjects() {
        this.$get(api.getsubjects + "/" + this.id).then(res => {
