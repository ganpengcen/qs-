  ...
  <div class="upper">
    ...
    <el-col :span="6">
      <el-card class="box-card">
        ...
        <!-- 添加 -->
        <el-form label-position="top" label-width="120px" :model="form" v-show="!id">
        ...
        <!-- 修改 -->
        <el-form label-position="top" label-width="120px" :model="editForm" v-show="id">
          <el-form-item label="监控对象" v-show="id">
            <el-input v-model="editForm.entityName" disabled></el-input>
          </el-form-item>
          <el-form-item label="围栏名称(必填)" v-show="id">
            <el-input v-model="editForm.name"></el-input>
          </el-form-item>
          <el-form-item label="围栏类型(必填)" v-show="id">
            <el-select
              disabled
              v-model="editForm.fenceType"
              filterable
              placeholder="请选择"
              @change="changePageControl(editForm.fenceType)"
              style="width:100%"
            >
              <el-option
                v-for="item in options"
          ...
          <el-form-item label="圆心坐标" v-show="editForm.fenceType==1">
            <el-input v-model="coordinateEdit" disabled></el-input>
          </el-form-item>
          <el-form-item label="围栏半径" v-show="editForm.fenceType==1">
            <el-input v-model="editForm.radius" placeholder="0-5000米" disabled></el-input>
          </el-form-item>
          <el-form-item label="行政区域" v-show="editForm.fenceType==4">
            <el-input
              v-model="editForm.district"
              @keyup.enter.native="getBoundary(editForm.district)"
            ></el-input>
          </el-form-item>
          <el-form-item label="偏移距离" v-show="editForm.fenceType==3">
            <el-input-number v-model="editForm.offset" :min="0" label></el-input-number>
          </el-form-item>
          <el-form-item label="围栏去噪参数">
            <el-input v-model="editForm.denoise"></el-input>
          </el-form-item>
          <el-form-item>
            <el-button type="danger" @click="save(editForm)">保存</el-button>
            <el-button type="success" @click="clearAll">清空&重画</el-button>
...
export default {
  ...
  computed: {
    ...
    coordinateEdit() {
      if (!(this.editForm.lng && this.editForm.lat)) {
        return null;
      } else {
        return this.editForm.lng + "," + this.editForm.lat;
      }
    }
  },
  watch: {},
  methods: {
    ...
    async GetFenceDetails() {
      try {
        ...
        this.editForm = res;
        this.changePageControl(this.editForm.fenceType);
        this.drawingOverlayEdit(this.editForm);
      } catch (e) {
        console.log(e);
      }
    },
